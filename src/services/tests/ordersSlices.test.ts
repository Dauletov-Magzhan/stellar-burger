...
import ordersSlices, { closeOrderModalData, fetchFeeds, fetchOrderBurger, fetchOrderByNumber, fetchOrders, selectOrderModalData, selectOrderRequest, selectOrders, selectOrdersToday, selectOredersTotal, selectUserOrders } from "../slices/ordersSlices"
...
describe('Тест OrdersSlices', () => {
    ...
    it('Тест асинхронного экшена fetchOrderBurger.fulfilled', () => {
        const action = { type: fetchOrderBurger.fulfilled.type, payload: { order: {_id: '1', name: 'burger'}}}
        const state = ordersSlices(store.getState(), action)
    ...
    it('Тест асинхронного экшена fetchFeeds.rejected', () => {
        const action = { type: fetchFeeds.rejected.type, error: { message: 'Error' }}
        const state = ordersSlices(store.getState(), action)
