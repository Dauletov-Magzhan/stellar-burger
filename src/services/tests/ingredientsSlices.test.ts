...
import {
  ...
  mockIngredient,
  mockStore
} from '../mocks/ingredientsSlicesMocks';
...
describe('Тест ingredientsSlices', () => {
  ...
  it('Тест на удаление ингредиента с экшеном "removeIngredient"', () => {
    const store = initialStore();
    const beforeRemove = selectConstructorItems(store.getState()).ingredients
      .length;
    store.dispatch(removeIngredient(mockIngredient));
    const afterRemove = selectConstructorItems(store.getState()).ingredients
      .length;
  ...
  it('Тест на изменение порядка ингредиента в конструкторе c экшеном "moveDownIngredient"', () => {
    const store = initialStore();
    const moveIngredient = selectConstructorItems(store.getState())
      .ingredients[0];
    store.dispatch(moveDownIngredient(moveIngredient));
    expect(selectConstructorItems(store.getState()).ingredients[1]).toEqual(
      moveIngredient
    );
  });
